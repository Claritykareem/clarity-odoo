...
class WebsiteSaleExt(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        if discount:
            ...
            res.qcontext.update(products=request.env['product.template'].browse(
                list(map(lambda p: p.get('product_template_id'), product_prices_data))))
        ...
        if attrib_values:
            ...
            for attribute in res.qcontext.get('attributes'):
                ...
                modified_list = [sublist for sublist in attrib_values_list]
                ...
                fuzzy_search_terms, product_counts, search_products = self._shop_lookup_products(attrib_set, options,
                                                                                                 post,
                                                                                                 search, website)
        ...
        if not res.qcontext.get('brand_val', False):
    ...
    def get_category_count_details(self, product_ids, category, attrib_values, min_price, max_price, conversion_rate,
                                   website, attrib_set, search, **post):
        category_count = {}
    ...
    def get_tag_count_details(self, product_ids, category, attrib_values, min_price, max_price, conversion_rate,
                              website, attrib_set, search, **post):
        tags_dict = None
...
class WebsiteExt(main.Website):
    ...
    def get_see_all(self, attr_id='', is_mobile='', is_tag='', category_id=''):
        category = None
        if category_id:
        ...
        if attr_id and attr_id != '0':
            ...
            if is_mobile == 'True':
                response = http.Response(template="theme_clarico_vega.see_all_attr_mobile",
                                         qcontext={'attributes': attributes, 'category': category})
            ...
            if is_mobile == 'True':
                response = http.Response(template="theme_clarico_vega.see_all_attr_brand_mobile",
                                         qcontext={'brands': brands, 'category': category})
            else:
            ...
            response = http.Response(template="theme_clarico_vega.see_all_tags",
                                     qcontext={'all_tags': ProductTag, 'category': category})
